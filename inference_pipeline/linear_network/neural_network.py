...
class NeuralNetwork:
    ...
    def create_network(self, data):
        ...
        input_layer_size = len(data.iloc[0]) - 1 # number of features
        ...
        output_layer_size = len(data['target'].unique()) # number of classes
        self.network = [input_layer_size] + hidden_layers_size + [output_layer_size]
        self.weights = []
        self.deltas = []
        self.gradients = []
        ...
        for i in range(1, len(self.network)):
            self.biases.append(np.zeros(self.network[i]))
